        ...
        **kwargs: dict,
        ...
        if jql:
            ...
            if max_results:
                ...
                # A page Jira did not have to truncate is the whole answer, so it
                # already carries the total and saves the count request below. Jira
                # marks a truncated page with a nextPageToken; a page filled to
                # max_results is treated as truncated too, in case an older
                # deployment leaves the token out, so a capped number is never
                # reported as the total.
                if not page.get("nextPageToken") and len(issues) < max_results:
                    total = len(issues)

            if total is None:
                # The search/jql endpoint reports no total of its own, so the count
                # comes from the endpoint next to it. On-prem still has the older
                # /search, which answers both in one request.
