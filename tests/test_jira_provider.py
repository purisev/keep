...
class TestJiraProvider:
    ...
    def test_jira_query_with_jql(self, mock_get, mock_post, jira_provider):
        ...
        # Cloud no longer reports a total from the search endpoint, so the count
        # comes from approximate-count and no issues are fetched at all.
    ...
    def test_jira_query_with_jql_counts_a_page_filled_to_the_brim(
        ...
        mock_get.return_value.ok = True
        # no nextPageToken, which an older deployment may leave out, so a full page
        # is counted rather than reported as a total capped at max_results
    ...
    @patch("requests.post")
    @patch("requests.get")
    def test_jira_query_with_ticket_id_still_works(
        self, mock_get, mock_post, jira_provider
    ):
        """Test that the jql branch does not intercept existing ticket_id queries"""
        mock_get.return_value.ok = True
